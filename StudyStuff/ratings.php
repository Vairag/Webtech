...
class ratings {
	...
	function __construct($wid) {
		...
		$this->widget_id = substr($wid, 1);
		
// 		require_once 'config.php';
// 		if(config())
// 		{
// 			// fetches the four latest attachments from db (with their ratings)
// 			$rating_query = "SELECT AVG(rating) AS rating FROM `ratings`
// 				WHERE id = $this->widget_id GROUP BY A.id";
// 			$query_result = mysql_query($rating_query);
...
// 		}
	...
	public function vote() {
		...
		if(config())
		{
			...
			// fetches the four latest attachments from db (with their ratings)
			$insert = "INSERT INTO `db11005921-gallery`.`ratings`
			(`id`, `attachment_id`, `course_id`, `rating`) VALUES
			(NULL, '$ID', NULL, '$vote');";
			mysql_query($insert);
		}
